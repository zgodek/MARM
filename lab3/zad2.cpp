...
namespace app {
    ...
    void parse_command() {
        ...
        if (strcmp(cmd_buffer, "help") == 0) {
            ...
            send_string_polling("led [3-6] off  - Wylacz diode\r\n");
            send_string_polling("btn            - Stan przycisku USER\r\n");
            send_string_polling("mem            - Wolna pamiec RAM\r\n");
            send_string_polling("cpu            - Uzycie procesora\r\n");
        }
        // 2. Komenda: btn (przycisk)
        else if (strcmp(cmd_buffer, "btn") == 0) {
            if (get_button_state()) send_string_polling("USER Button: WCISNIETY\r\n");
            else                    send_string_polling("USER Button: PUSZCZONY\r\n");
        }
        ...
        // 5. Komendy LED: "led 3 on", "led 5 off"
            ...
            if (led_num >= 3 && led_num <= 6) {
                if (strstr(cmd_buffer, " on")) {
                    ...
                    send_string_polling("Blad: uzyj 'on' lub 'off'\r\n");
                }
            } else {
            ...
            if(strlen(cmd_buffer) > 0)
                send_string_polling("Nieznana komenda. Wpisz 'help'.\r\n");
        }
    ...
    void test_thread(void*) {
        isix::wait_ms(100); 
        send_string_polling("\r\n=== STM32 ISIX SHELL ===\r\n");
        send_string_polling("Wpisz 'help' aby zobaczyc liste komend.\r\n> ");
